...
- Good fit: `/ai-slop-cleaner src/auth/middleware.ts src/auth/utils.ts`
- Good fit: cleanup scoped to only the files changed in a prior session
...
- **Palette rationale:** challenge default AI blue/purple palettes (e.g., Tailwind `#3B82F6`) when no brand or system rationale exists.
- **Layout rhythm:** avoid overly perfect 3- or 4-column uniform grids when asymmetry or varied card weights would better serve the product.
